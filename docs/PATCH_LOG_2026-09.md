...
- 새 파일 `lib/gridLayout.ts`: `estimateVisionTokens(width, height)` — 문서화된 계산식(장변 2048 → 단변 768 → 512px 타일) 기반 추정치. gpt-5.6 계열의 정확한 값은 공개되어 있지 않아 **추정치**이며 실제 청구 토�큰과 다를 수 있음
