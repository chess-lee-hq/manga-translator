...
function mapGridToResults(translations: GridTranslationResult[], cells: GridCellInfo[], width: number, height: number): RawTranslationResult[] {
  return translations.flatMap(t => {
    const cell = cells.find(c => c.id === t.id);
    return cell ? [{ original_text: t.original_text, translated_text: t.translated_text, box_2d: toBox2d(cell.box, width, height) }] : [];
  });
}
...
export async function translatePage(img: UploadedImage, settings: TranslationSettings): Promise<TranslationResult[]> {
  ...
  const { provider, googleKey, openaiKey, geminiVersion, openAiVersion, glossary } = settings;
  ...
  if (textBoxes.length === 0) {
    ...
    if (provider === 'google') {
      ...
      rawResults = await translateMangaImageOpenAI(openAiVersion, openaiKey, geminiResults, glossary);
    }
  } else {
    ...
    if (provider === 'openai') rawResults = await translateMangaImageOpenAI(openAiVersion, openaiKey, rawResults, glossary);
  }
...
export async function translateRegion(img: UploadedImage, box: Box2d, settings: TranslationSettings): Promise<{ originalText: string; translatedText: string }> {
  ...
  const [cell] = await translateGridImage(googleKey, base64Of(img.src), base64Of(gridResult.dataUrl), img.mimeType, 1, geminiVersion, glossary);
  ...
  return {
    originalText,
    translatedText: hasText ? await retranslateTextOpenAI(openAiVersion, openaiKey, originalText, glossary) : '인식된 텍스트가 없습니다.',
  };
}
...
export async function retranslateText(originalText: string, settings: TranslationSettings): Promise<string> {
  if (settings.provider === 'google') {
    ...
    return retranslateTextGemini(settings.googleKey, originalText, settings.geminiVersion, settings.glossary);
  }
  if (!settings.openaiKey) throw new Error('OpenAI API 키가 필요합니다.');
  return retranslateTextOpenAI(settings.openAiVersion, settings.openaiKey, originalText, settings.glossary);
}
