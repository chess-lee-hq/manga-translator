...
const MODEL_URL = new URL(`${import.meta.env.BASE_URL}manga109_yolo_s.onnx`, document.baseURI).href;
...
function getWorker(): Worker {
  ...
  created.onmessage = (event: MessageEvent<DetectResponse>) => {
    ...
    if (response.ok) {
      console.debug(`[yolo] 박스 ${response.boxes.length}개 검출 (${response.elapsedMs}ms)`);
...
export async function detectSpeechBubbles(image: HTMLImageElement, confThreshold = 0.25, iouThreshold = 0.45): Promise<BoundingBox[]> {
  ...
  return new Promise((resolve, reject) => {
    ...
    const request: DetectRequest = { id, bitmap, modelUrl: MODEL_URL, confThreshold, iouThreshold };
